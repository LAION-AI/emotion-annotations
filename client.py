...
def format_and_print_results(result: dict):
    print("\n" + "=" * 50)
    print(f"RESULTS FOR: {result.get('file', 'N/A')}")
    print("=" * 50)
    if "error" in result:
    ...
    if "emotions" not in result or not result["emotions"]:
        print("No emotion data returned from server.")
        print("=" * 50 + "\n")
        return
    ...
    sorted_attributes = sorted(
        attribute_scores.items(), key=lambda item: item[1], reverse=True
    )

    print("--- Top 5 Core Emotions ---")
    for emotion, score in sorted_emotions[:5]:
        print(f"- {emotion.replace('_', ' '):<35} | Score: {score:.3f}")
    print("\n--- Top 5 Attributes / Dimensions ---")
    for attr, score in sorted_attributes[:5]:
        print(f"- {attr.replace('_', ' '):<35} | Score: {score:.3f}")
    print(f"\n--- All {len(sorted_emotions)} Emotion Scores (descending) ---")
    for emotion, score in sorted_emotions:
        print(f"- {emotion.replace('_', ' '):<40} {score:7.3f}")
    print(f"\n--- All {len(sorted_attributes)} Attribute Scores (descending) ---")
    for attr, score in sorted_attributes:
        print(f"- {attr.replace('_', ' '):<40} {score:7.3f}")
    print("=" * 50 + "\n")


async def query_single_file(
    session: aiohttp.ClientSession, file_path: Path, url: str
) -> dict:
    try:
        with open(file_path, "rb") as f:
            data = aiohttp.FormData()
            data.add_field(
                "file", f, filename=file_path.name, content_type="audio/mpeg"
            )
            async with session.post(url, data=data) as response:
                if response.status == 200:
                    json_res = await response.json()
                    json_res["file"] = file_path.name
                    return json_res
                else:
                    return {
                        "file": file_path.name,
...
async def main():
    parser = argparse.ArgumentParser(
        description="Enhanced client for the Emotion Annotation API.",
    ...
    group.add_argument(
        "--demo",
        action="store_true",
        help="Run inference on the cached demo file and print results.",
    )
    group.add_argument(
        "--benchmark",
        action="store_true",
        help="Run a high-throughput benchmark with 128 concurrent requests.",
    )
    group.add_argument(
        "--file", type=str, help="Path to a single audio file to process."
    )
    group.add_argument(
        "--folder", type=str, help="Path to a folder of audio files to process."
    )
    parser.add_argument(
        "--url",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Number of concurrent requests for folder/benchmark modes.",
    )
    ...
    if args.demo:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            result = await query_single_file(session, demo_file_path, args.url)
            end_time = time.time()
            format_and_print_results(result)
            print(f"Time for single inference: {end_time - start_time:.2f} seconds.")
    elif args.benchmark:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"Creating {num_copies-1} temporary copies for benchmark...")
            files_to_process = [demo_file_path] + [
                shutil.copy(
                    demo_file_path, temp_path / f"copy_{i}_{demo_file_path.name}"
                )
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                tasks = [
                    query_single_file(session, file, args.url)
                    for file in files_to_process
                ]
                results = await asyncio.gather(*tasks)
            end_time = time.time()
            total_time, success_count = end_time - start_time, sum(
                1 for r in results if "error" not in r
            )
            print("\n--- Benchmark Complete ---")
            print(f"Total time to process {num_copies} files: {total_time:.2f} seconds")
            print(
                f"Throughput: {success_count / total_time:.2f} files/second"
                if total_time > 0
                else "N/A"
            )
            print(
                f"Effective time per file: {total_time / success_count:.3f} seconds"
                if success_count > 0
                else "N/A"
            )
    else:
        files_to_process = (
            ...
            if args.file
                ...
                p
                for p in Path(args.folder).rglob("*")
        ...
        for res in results[:3]:
            format_and_print_results(res)
        if len(results) > 3:
            print(f"...and {len(results) - 3} more results not shown.")
