...
MAX_AUDIO_SAMPLES = (
    int(TARGET_SAMPLE_RATE * MAX_AUDIO_DURATION_SEC) if MAX_AUDIO_DURATION_SEC else None
)
...
try:
    ...
    print(
        f"Detailed ImportError when trying to import 'EmbeddingsModel' from 'spk_embeddings.py': {e}"
    )
    print("\nThis could be because:")
    print(
        "1. spk_embeddings.py is not in the current path (less likely if it's in the same directory)."
    )
    print(
        "2. A module that spk_embeddings.py itself tries to import is missing or cannot be imported."
    )
    print(
        "   Common dependencies for spk_embeddings.py are: torch, torchaudio, transformers, huggingface_hub."
    )
...
def find_audio_files(folder_path: Path, extensions: tuple) -> list:
    ...
    audio_files = []
    print(
        f"Scanning for audio files with extensions {extensions} in '{folder_path}'..."
    )
    for ext in extensions:
        audio_files.extend(list(folder_path.rglob(f"*{ext.lower()}")))
        audio_files.extend(
            list(folder_path.rglob(f"*{ext.upper()}"))
        )  # Case-insensitive
...
def load_and_resample_audio(file_path: Path, target_sr: int):
    ...
    try:
        ...
        if waveform.shape[0] > 1:  # If more than 1 channel
            ...
            print(
                f"Warning: Audio file {file_path} has 0 channels after loading. Skipping."
            )
            return None, None
...
def worker_process_files(
    ...
    device = torch.device(
        f"cuda:{0}"
    )  # After CUDA_VISIBLE_DEVICES, cuda:0 is the target GPU
    ...
    try:
        ...
        print(
            f"[Process {process_id} on GPU {gpu_id}] Error loading model '{model_name}': {e}"
        )
    ...
    for i in range(0, len(assigned_file_paths), gpu_batch_size):
        ...
        waveforms_for_batch = []
        valid_paths_in_chunk = (
            []
        )  # To keep track of which files loaded successfully for this chunk
        max_len_in_chunk = 0
        ...
        for audio_file_path in current_chunk_paths:
            ...
            waveforms_for_batch.append(
                waveform.squeeze(0)
            )  # Squeeze to (num_samples) for padding
            valid_paths_in_chunk.append(audio_file_path)
            if waveform.shape[1] > max_len_in_chunk:
                max_len_in_chunk = waveform.shape[1]

        if (
            not waveforms_for_batch
        ):  # All files in this chunk were skipped or failed loading
            continue
        ...
        try:
            ...
            print(
                f"[Process {process_id} on GPU {gpu_id}] Error processing batch (starts with {valid_paths_in_chunk[0].name if valid_paths_in_chunk else 'N/A'}): {e}"
            )
            ...
            for (
                ...
                valid_paths_in_chunk
            ):  # These were files that loaded but failed during model inference/saving
                progress_queue.put(1)
...
if __name__ == "__main__":
    ...
    if not files_to_process:
        print(
            "All audio files seem to have corresponding .npy embeddings already. Exiting."
        )
        sys.exit(0)
    ...
    if torch.cuda.is_available():
        ...
        if not torch.cuda.is_available():  # Re-check for clarity
            print(
                "ERROR: No CUDA GPUs found. This script requires CUDA for efficient operation."
            )
            print(
                "Please install PyTorch with CUDA support or adapt the script for CPU use."
            )
            sys.exit(1)
    ...
    print(
        f"Max audio samples per file (after {TARGET_SAMPLE_RATE}Hz resample): {MAX_AUDIO_SAMPLES or 'No limit'}\n"
    )

    for i in range(num_gpus):
        if not worker_assignments[
            i
        ]:  # If a GPU has no files assigned (e.g., fewer files than GPUs)
            continue
    ...
    with tqdm(
        total=len(files_to_process), desc="Generating Embeddings", unit="file"
    ) as pbar:
    ...
    print(
        f"Embeddings (.npy files) are saved alongside their original audio files in '{INPUT_FOLDER}'."
    )
