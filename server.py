...
class FullEmbeddingMLP(nn.Module):
    def __init__(
        ...
        if len(mlp_dropout_rates) != len(mlp_hidden_dims) + 1:
            raise ValueError(f"Dropout rates length error.")
...
def load_models_and_processor():
    ...
    if USE_FLASH_ATTENTION_2 and DEVICE != "cpu":
        try:
            ...
            logging.warning(
                f"Flash Attention 2 is enabled but failed to load. Error: {e}"
            )
    ...
    logging.info(
        f"Downloading MLP models from {HF_MLP_REPO_ID} to {LOCAL_MLP_MODELS_DIR}..."
    )
    snapshot_download(
        repo_id=HF_MLP_REPO_ID,
        local_dir=LOCAL_MLP_MODELS_DIR,
    ...
    for model_path in mlp_files:
        ...
        dimension_name = (
            "_".join(parts[1:-1]) if "best" in parts[-1] else "_".join(parts[1:])
        )
    ...
    def process_batch(self, audio_paths: List[Path]) -> List[Dict[str, Any]]:
        ...
        for path in audio_paths:
            final_results.append(
                result_map.get(
                    path.name, {"file": path.name, "error": "Processing failed"}
                )
            )
    ...
    def _load_and_process_audio(self, audio_path: Path):
        try:
            waveform, _ = librosa.load(
                audio_path, sr=SAMPLING_RATE, mono=True, duration=MAX_AUDIO_SECONDS
            )
...
class DynamicBatcher:
    def __init__(
        self,
        inference_manager: BatchInferenceManager,
        batch_size: int,
        max_wait_time: float,
    ):
        self.inference_manager, self.batch_size, self.max_wait_time = (
            inference_manager,
            batch_size,
            max_wait_time,
        )
        ...
        self.processing_thread = threading.Thread(
            target=self._batching_loop, daemon=True
        )
        self.loop = None
    ...
    def start(self):
        ...
        logging.info(
            f"Dynamic batcher started with batch_size={self.batch_size}, max_wait_time={self.max_wait_time}s"
        )
    ...
    async def add_request(self, temp_path: Path) -> Dict:
        ...
        request = PendingRequest(future=future, temp_path=temp_path)
        with self.lock:
            self.queue.append(request)
        try:
            return await asyncio.wait_for(future, timeout=MAX_AUDIO_SECONDS + 15)
        ...
        while not self.shutdown_event.is_set():
            ...
            with self.lock:
                if self.queue and (
                    ...
                    or time.time() - self.queue[0].enqueue_time > self.max_wait_time
                ):
                    batch_to_process = self.queue[:]
                    self.queue.clear()
            if batch_to_process:
                self._process_and_respond(batch_to_process)
            else:
                time.sleep(0.005)

    def _process_and_respond(self, batch: List[PendingRequest]):
        batch_paths = [req.temp_path for req in batch]
        logging.info(f"Processing batch of size {len(batch_paths)}...")
        try:
            results = self.inference_manager.process_batch(batch_paths)
            result_map = {Path(res["file"]).name: res for res in results}
            for request in batch:
                result = result_map.get(
                    request.temp_path.name, {"error": "Processing failed."}
                )
...
async def lifespan(app: FastAPI):
    global batcher
    processor, whisper_model, mlps = load_models_and_processor()
    inference_manager = BatchInferenceManager(processor, whisper_model, mlps)
    batcher = DynamicBatcher(inference_manager, batch_size=16, max_wait_time=0.05)
    batcher.start()
    yield
    batcher.stop()
...
@app.post(
    "/predict", summary="Analyze a single audio file for emotion and transcription"
)
async def predict_emotion(file: UploadFile = File(...)):
    ...
    result = await batcher.add_request(temp_path)
    return JSONResponse(content=result)


@app.get("/health", summary="Health check endpoint")
async def health_check():
    return {"status": "ok", "device": DEVICE, "models_loaded": batcher is not None}
